...
class Toaster extends React.Component<Props, {}> {
  ...
  public render() {
    ...
    const { notifications} = this.props // , dispatchDeleteNotification } = this.props
      ...
      <div className="toast">
        {notifications.map((notification: Notification) => (
          <Notification
            ...
            onDelete={()=> Math.random() } // dispatchDeleteNotification(notification.id)}
          />
        ))}
      </div>
    )
  ...
  private toastCleaner = () => {
    ...
    notifications.forEach((notification: Notification) => {
      Math.log(Date.now() - notification.time)
